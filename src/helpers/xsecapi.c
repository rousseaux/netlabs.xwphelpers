...
APIRET xsecQueryAllUsers(PULONG pcUsers,               // ou: array item count
...
/*
 *@@ xsecQueryUserName:
 *      convenience function that returns a single user name.
 *
 *      Required authority: none. We consider plain user names
 *      something not worth protecting.
 *
 *@@added V1.0.2 (2003-11-13) [umoeller]
 */

APIRET xsecQueryUserName(XWPSECID uid,      // in: user ID
                         PSZ pszUserName)   // out: user name (buffer must be XWPSEC_NAMELEN in size)
{
    APIRET              arc = NO_ERROR;
    PXWPSHELLCOMMAND    pCommand;

    if (!(arc = CreateXWPShellCommand(QUECMD_QUERYUSERNAME,
                                      &pCommand)))
    {
        PQUEUEUNION pUnion = &pCommand->pShared->Data;

        pUnion->QueryUserName.uid = uid;

        if (!(arc = SendXWPShellCommand(pCommand)))
            strlcpy(pszUserName,
                    pUnion->QueryUserName.szUserName,
                    XWPSEC_NAMELEN);
...
APIRET xsecQueryProcessOwner(USHORT pid,            // in: process ID or 0 for caller
                             XWPSECID *puid)        // out: user ID
{
                                      ...
                                      &pCommand)))
    {
        ...
        pUnion->QueryProcessOwner.pid = pid;

        if (!(arc = SendXWPShellCommand(pCommand)))
            *puid = pUnion->QueryProcessOwner.uid;
...
/*
 *@@ xsecSwitchUser:
 *      switches the credentials for the current (calling)
 *      process. This allows a process to switch to a different
 *      user, for example to implement a "su" command, since
 *      subprocesses started after the switch will inherit the
 *      new credentials.
 *
 *      Note that switching the user does not make the new
 *      user a "local" user. Neither will switching users
 *      adjust the environment of the current process; to
 *      implement shell-like behavior, the caller must adjust
 *      the environment variables such as "USER" and "HOME"
 *      by itself.
 *
 *      Required authority: none.
 *
 ...
 *
 *      If NO_ERROR is returned, the credentials of the current
 *      process have been switched.
 */

APIRET xsecSwitchUser(PCSZ pcszUserName,
                      PCSZ pcszPassword)
{
    APIRET              arc = NO_ERROR;
    PXWPSHELLCOMMAND    pCommand;

    if (!(arc = CreateXWPShellCommand(QUECMD_SWITCHUSER,
                                      &pCommand)))
    {
        PQUEUEUNION pUnion = &pCommand->pShared->Data;

        if (    (!(arc = strhCopyBuf(pUnion->SwitchUser.szUserName,
                                     pcszUserName,
                                     sizeof(pUnion->SwitchUser.szUserName))))
             && (!(arc = strhCopyBuf(pUnion->SwitchUser.szPassword,
                                     pcszPassword,
                                     sizeof(pUnion->SwitchUser.szPassword))))
             && (!(arc = SendXWPShellCommand(pCommand)))
           )
        {
