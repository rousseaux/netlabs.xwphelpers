...
static
int EXPATENTRY sb_byteToAscii(const ENCODING * enc, const char *p)
{
    return *p;
}
#else
...
static
int EXPATENTRY sb_charMatches(const ENCODING * enc, const char *p, int c)
{
    return *p == c;
}
#else
...
static int  EXPATENTRY isSpace(int c)
{
    switch (c)
    {
        case 0x20:
