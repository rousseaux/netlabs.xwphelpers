...
typedef int (* EXPATENTRY XML_ExternalEntityRefHandler)(XML_Parser parser,
...
/*
 *@@ XML_Encoding:
 *      structure passed with the UnknownEncodingHandler (see
 *      XML_SetUnknownEncodingHandler).
 *
 *      The map array contains information for every possible
 *      possible leading byte in a byte sequence.
 *
 *      --  If the corresponding value is >= 0, then it's a single byte
 *          sequence and the byte encodes that Unicode value.
 *
 *      --  If the value is -1, then that byte is invalid as the
 *          initial byte in a sequence.
 *
 *      --  If the value is -2, -3, or -4, then the byte is the beginning
 *          of a 2, 3, or 4 byte sequence, respectively. The actual
 *          conversion is then accomplished by a call to the function
 *          pointed at by "convert". This function should return the
 *          Unicode scalar value for the sequence or -1 if the sequence
 *          is malformed.
 *
 *          The "convert" pointer may be null if there are only
 *          single byte codes. The data parameter passed to the
 *          convert function is the data pointer from
 *          XML_Encoding. The string s is NOT null terminated and
 *          points at the sequence of bytes to be converted.
 *
 *      The function pointed at by "release" is called by the parser
 *      when it is finished with the encoding. It may be null.
 */
